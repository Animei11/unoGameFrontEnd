// var cardArray = ['0R', '1R', '1R', '2R', '2R', '3R', '3R', '4R', '4R', '5R', '5R', '6R', '6R', '7R', '7R', '8R', '8R', '9R', '9R', 'skipR', 'skipR', '_R', '_R', 'D2R', 'D2R',
// '0G', '1G', '1G', '2G', '2G', '3G', '3G', '4G', '4G', '5G', '5G', '6G', '6G', '7G', '7G', '8G', '8G', '9G', '9G', 'skipG', 'skipG', '_G', '_G', 'D2G', 'D2G',
// '0B', '1B', '1B', '2B', '2B', '3B', '3B', '4B', '4B', '5B', '5B', '6B', '6B', '7B', '7B', '8B', '8B', '9B', '9B', 'skipB', 'skipB', '_B', '_B', 'D2B', 'D2B',
// '0Y', '1Y', '1Y', '2Y', '2Y', '3Y', '3Y', '4Y', '4Y', '5Y', '5Y', '6Y', '6Y', '7Y', '7Y', '8Y', '8Y', '9Y', '9Y', 'skipY', 'skipY', '_Y', '_Y', 'D2Y', 'D2Y',
// 'W', 'W', 'W', 'W', 'D4W', 'D4W', 'D4W', 'D4W'];

// var cardImageArray = ["red", "green", "blue", "yellow"];

function uno() {
    prompt("hello");
}

function drawCards() {
    prompt("rip");
}

// Displays the number of cards someone has (Maybe there's a way to do this I don't know)
function displayOpponentsCards(handSize) {
    var img = "cards/back.png";
    for (i = 0; i < handSize; i++) {
        document.body.appendChild(img);
    }
}

// Changes background color not image, works with color but not image for some reason
function colorChange(color) {
    switch(color) {
        case "red":
            document.body.style.background = "#87140e";
            break;
        case "green":
            document.body.style.background = "#0d522b";
            break;
        case "blue":
            document.body.style.background = "#0f34a3";
            break;
        case "yellow":
            document.body.style.background = "#ada30a";
            break;
    }
    document.body.style.backgroundImage =  url("bgpics/red.png");
}